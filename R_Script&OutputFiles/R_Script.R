...
setwd("C:/Local Disk D/OneDrive - Universiti Teknologi PETRONAS/16 Workshops and Tutorials/Coursera_GoogleDataAnalyticsCertification/CapstoneProject/Data_CyclistCaseStudy/Last12MonthsData") 
#sets your working directory to
# simplify calls to data ... make sure to use your OWN username instead of mine ;)
...
# Let's see structure of each data. Inspect the dataframes and look for incongruencies
str(m8_2021)
str(m9_2021)
str(m10_2021)
str(m11_2021)
...
all_trips_1 <- all_trips %>% mutate(member_casual = recode(member_casual, 
                                                           "Subscriber" = "member",
                                                           "Customer" = "casual"))
# Check to make sure the proper number of observations were reassigned
table(all_trips_1$member_casual)
...
all_trips_1$date <- as.Date(all_trips_1$started_at) #The default format is yyyy-mm-dd
all_trips_1$month <- format(as.Date(all_trips_1$date), "%m")
all_trips_1$day <- format(as.Date(all_trips_1$date), "%d")
all_trips_1$year <- format(as.Date(all_trips_1$date), "%Y")
all_trips_1$day_of_week <- format(as.Date(all_trips_1$date), "%A")

# Add a "ride_length" calculation to all_trips (in seconds)
# https://stat.ethz.ch/R-manual/R-devel/library/base/html/difftime.html
all_trips_1$ride_length <- difftime(all_trips_1$ended_at, all_trips_1$started_at)

# Inspect the structure of the columns
str(all_trips_1)

# Convert "ride_length" from Factor to numeric so we can run calculations on the data
is.factor(all_trips_1$ride_length)
all_trips_1$ride_length <- as.numeric(as.character(all_trips_1$ride_length))
is.numeric(all_trips_1$ride_length)
...
all_trips_v2 <- all_trips_1[!(all_trips_1$start_station_name == "HQ QR" | all_trips_1$ride_length<0),]
...
#  change the file location accordingly (most likely "C:\Users\YOUR_USERNAME\Desktop\...") to export the data. 
# You can read more here: https://datatofish.com/export-dataframe-to-csv-in-r/
counts <- aggregate(all_trips_v2$ride_length ~ all_trips_v2$member_casual +
                        all_trips_v2$day_of_week, FUN = mean)
write.csv(counts, file = "C:/Local Disk D/OneDrive - Universiti Teknologi PETRONAS/16 Workshops and Tutorials/Coursera_GoogleDataAnalyticsCertification/CapstoneProject/Data_CyclistCaseStudy/Last12MonthsData/avg_ride_length.csv")
